...
from Actions import MenuCreator,Start
...
class main_win():
	...
	def calc_posizioni(self):
		...
		while n<4:
			...
			i=0
			while i<7:
				self.pos_sce[n].append([(i+1)*5+i*w,(n+1)*g+n*h])
				i=i+1
			n=n+1
	...
	def scelta_presa(self, obj, prese_possibili):
		...
		while n<len(prese_possibili):
			...
			while i < len(prese_possibili[n]):
				...
				actor.set_position(self.pos_sce[n][i][0],self.pos_sce[n][i][1])
				actor.set_reactive(True)
				self.stage.add_actor(actor)
				actor.connect('button-press-event',self.scelta_fatta,oggetti,obj,n,index)
